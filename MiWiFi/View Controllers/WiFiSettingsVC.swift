...
class WiFiSettingsVC: UIViewController, WiFiSettingsVCDelegate {
	...
	private let settingsArr: [Settings] = [Settings(title: "FaceID",
													iconName: "faceid",
													color: .systemBlue,
													renderingMode: .alwaysOriginal,
													config: .standardConfiguration)]

	private let creditArr: [Settings] = [Settings(title: "Acknowledgements",
												  iconName: "qrcode",
												  color: .miAddButtonColor,
												  renderingMode: .alwaysOriginal,
												  config: .standardConfiguration)]
	...
	private func configureSettingsTableView() {
		...
		miSettingsTableView.register(SettingsCell.self, forCellReuseIdentifier: reuseIdentifier)
	}
}
...
extension WiFiSettingsVC: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, titleForFooterInSection section: Int) -> String? {
		switch section {
		case 0:
			return "Having FaceID enabled ensures that your WiFi passwords are safe. Your passwords are only stored on your device."
		case 1:
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		case 0:
			cell.textLabel?.text = settingsArr[indexPath.row].title
